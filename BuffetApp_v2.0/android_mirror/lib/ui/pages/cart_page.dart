            ...
            child: ListView.separated(
              ...
              itemBuilder: (ctx, i) {
                ...
                return ListTile(
                  ...
                  trailing: Text(formatCurrency(it.subtotal), style: const TextStyle(fontWeight: FontWeight.bold)),
                  leading: Row(mainAxisSize: MainAxisSize.min, children: [
                    IconButton(onPressed: () async {
                      final db = await AppDatabase.instance();
                      final row = (await db.rawQuery('SELECT stock_actual FROM products WHERE id=?', [it.productoId])).first;
                      final stock = (row['stock_actual'] as int?) ?? 0;
                      if (stock != 999) {
                        await db.rawUpdate('UPDATE products SET stock_actual = stock_actual + 1 WHERE id = ?', [it.productoId]);
                      }
                    ...
                    IconButton(onPressed: () async {
                      ...
                      final updated = await db.rawUpdate('UPDATE products SET stock_actual = stock_actual - 1 WHERE id = ? AND stock_actual > 0', [it.productoId]);
                      if (updated == 0) {
                        // ignore: use_build_context_synchronously
            ...
            child: Column(
              ...
              children: [
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: cart.isEmpty ? null : () async {
                        ...
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Carrito limpiado')));
                        }
                      },
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text(formatCurrency(cart.total), style: const TextStyle(fontWeight: FontWeight.bold)),
                ]),
                const SizedBox(height: 10),
                ElevatedButton(
                  ...
                  child: Text('COBRAR  ${formatCurrency(cart.total)}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                )
              ],
            ),
          )
